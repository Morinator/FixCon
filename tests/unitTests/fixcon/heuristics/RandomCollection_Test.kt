...
internal class RandomCollection_Test {
    ...
    fun asymptoticWeighting_test() {
        val rCol = RandomCollection(mapOf(0 to 1.0, 1 to 2.0))
        val runs = 1_000_000
    ...
    fun negativeWeight_Exception_test() {
        assertThrows(IllegalArgumentException::class.java) { RandomCollection(mapOf(0 to 1.0, 1 to -2.0)) }
    }

    @Test
    fun stringCollection_test() {
        val animals = setOf("Hund", "Katze", "Giraffe")
        val rCol = RandomCollection(mapOf("Hund" to 1.4, "Katze" to 2.5, "Giraffe" to 23.6))
        repeat(10) {rCol.random in animals}
    }

    @Test
    fun acceptsMixedTypes_test() {
        val rCol = RandomCollection(mapOf(1 to 1.2, "hallo" to 1.5))
        repeat(5) { assertTrue(rCol.random in setOf(1, "hallo"))}
    }

    @Test
    fun weightZeroNeverReturned_test() {
        val rCol = RandomCollection(mapOf("immer" to 0.1, "nie" to 0.0))
