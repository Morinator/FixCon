...
class LowerBoundGenerator(var problem: CFCO_Problem) {
    ...
    fun getBound(runs: Int = getRunCount()): Solution {
        ...
        val bestLaplaceSolution = takeBestSolution(LaplacePicker)
        val bestDenseSolution = takeBestSolution(DensePicker)
        val bestSparseSolution = takeBestSolution(SparsePicker)
    ...
    private fun generateConnectedSubgraph(picker: VertexPicker): Solution {
        val startVertex = picker.startVertex(problem.originalGraph)
        ...
        while (subgraphSet.size != problem.targetSize) {
            ...
            val nextVertex = picker.extensionVertex(problem.originalGraph, subgraphSet, extensionSet)
