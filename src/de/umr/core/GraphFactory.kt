...
fun createClique(numVertices: Int) =
        vog.fromUnweightedEdges((0 until numVertices).flatMap { x -> (0 until x).map { y -> Pair(x, y) } })
...
fun createCircle(numVertices: Int) =
        vog.fromUnweightedEdges((0 until numVertices).map { Pair(it, ((it + 1) % numVertices)) })

/**@return The graph consists exclusively of one path. Thus it has exactly size-1 edges.*/
fun createPath(numVertices: Int) =
        vog.fromUnweightedEdges((0 until numVertices - 1).map { Pair(it, (it + 1)) })
...
fun createStar(numVertices: Int) =
        vog.fromUnweightedEdges((1 until numVertices).map { Pair(0, it) })
