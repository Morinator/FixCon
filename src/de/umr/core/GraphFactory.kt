...
fun createClique(numVertices: Int) = graph((0 until numVertices).flatMap { x -> (0 until x).map { y -> Triple(x, y, weight) } })

/**@return The graph consists exclusively of one circle (a path where the first and last vertex are equal).*/
fun createCircle(numVertices: Int) = graph((0 until numVertices).map { Triple(it, (it + 1) % numVertices, weight) })

/**@return The graph consists exclusively of one path. Thus it has exactly size-1 edges.*/
fun createPath(numVertices: Int) = graph((0 until numVertices - 1).map { Triple(it, it + 1, weight) })
...
fun createStar(numVertices: Int) = graph((1 until numVertices).map { Triple(0, it, weight) })
