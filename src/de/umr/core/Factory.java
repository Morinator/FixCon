...
public final class Factory {
    ...
    private static <T> void throw_error_if_edgeList_invalid(final List<T[]> edgeList) {
        if (edgeList.stream().anyMatch(x -> x.length != 2))
            throw new IllegalArgumentException("All elements must have 2 elements to represent an edge");
    }
}